...
while iteration <= Iteration:
    ...
    while k <= Time_horizon - 1:
        ...
        for i in Affect_by_O_t:  
            ...
            if len(Psi) > 0:
                permutation1 = itertools.product('0123', repeat=len(Psi))  # state realizations for Psi
        ...
        if k <= Index:  # let the disease run for $\ell$ days
            ...
            for i in range(N):
                if Node_list[i].Q == 0:
                    sum_budget = sum_budget + Node_list[i].v[0]  # the budget equals to the number of expected infectious nodes
            Budget = math.floor(sum_budget)
            q = sum_budget - Budget
        ...
        for i in range(N): 
            ...
            if Node_list[i].Q == 0:
                ...
                Sum_s = Sum_s + score
                Value_u.append(score)
                Index_u.append(i)
        sorted_Index = sorted(range(len(Value_u)), key=lambda q: Value_u[q], reverse=True)

        if Budget < len(Index_u):
            for item in range(Budget):
                Node_list[Index_u[sorted_Index[item]]].Inform = 1
        else:
            for item in Index_u:
                Node_list[item].Inform = 1

        for each in range(N):
            if Node_list[each].State in Infection_State_count:  # count infected nodes
